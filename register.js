  ...
  (async()=>{
    try{
      ...
      if(invDoc.exists()&&invDoc.data().status==='active'){
        ...
        INV_TOKEN=invId;
        INV_ROLE=invDoc.data().role||'agent';
        const badge=document.getElementById('reg-role-badge');
        if(INV_ROLE==='client'){
          badge.innerHTML='<i class="bi bi-person-fill me-1"></i>Client Registration';
          badge.style.background='#dbeafe';badge.style.color='#1e40af';
        }
      }else{
        msg('rmsg','Invalid or expired invite link!');
...
window.doRegister=async function(){
  ...
  if(!INV_TOKEN){msg('rmsg','Invalid invite link! Please use a valid invite link.');return;}
  const btn=document.getElementById('rbtn');
  btn.disabled=true;btn.innerHTML='<span class="sp me-2"></span>Creating...';
  try{
    const usnap=await getDocs(query(collection(db,'users'),where('username','==',u)));
    if(!usnap.empty){
      msg('rmsg','This username is already taken!');
      btn.disabled=false;btn.innerHTML='<i class="bi bi-person-check me-2"></i>Create Account';return;
    }
    ...
    await setDoc(doc(db,'users',cred.user.uid),{
      uid:cred.user.uid,email,username:u,name:n||u,
      role:INV_ROLE||'agent',status:'active',
      apiToken:tok,balance:0,totalOTP:0,totalEarning:0,paidOut:0,
      agentId,agentName,
      dailyTaken:{date:'',count:0},
      createdAt:Timestamp.now()
    });
    await updateDoc(doc(db,'invite_links',INV_TOKEN),{status:'used',usedBy:u,usedAt:Timestamp.now()});
    msg('rmsg',`✅ Account "${u}" created! Redirecting to login...`,'s');
    await signOut(auth);
    setTimeout(()=>window.location.href='/login',2000);
  }catch(e){
    msg('rmsg','Error: '+e.message);
    console.error(e);
  }
