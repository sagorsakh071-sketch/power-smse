...
<div class="aw"><div class="ac">
  ...
  <div class="mbf"><label class="fl"><i class="bi bi-shield-check me-1"></i>Verification</label><div class="capw"><div class="capb" id="lcap">?+?=?</div><input type="number" id="lcans" class="fc" placeholder="Answer"></div></div>
...
function initCap(){
  ...
  CAP=a+b;
  const el=document.getElementById('lcap');
  if(el)el.textContent=a+' + '+b+' = ?';
}
window.tEye=function(pi,ii){
  const p=document.getElementById(pi),i=document.getElementById(ii);
  if(!p||!i)return;
  p.type=p.type==='password'?'text':'password';
  i.className=p.type==='password'?'bi bi-eye-slash':'bi bi-eye';
};
window.onload=function(){
  initCap();
  const lyr=document.getElementById('lyr');
  if(lyr)lyr.textContent=new Date().getFullYear();
};
</script>
